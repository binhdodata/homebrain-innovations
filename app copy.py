...
original_data = None
Training_data = None
imputer = None  # You can initialize it later in your code
...
def train_model():
    global model, preprocessor, metrics, Training_data, original_data  # Modify this line to include original_data
    try:
        data = pd.read_csv('static/csv/Sold.csv')

        # Define the numerical columns here, outside of the nested function
        numerical_cols = [
            'Walk Score (out of 100)', 'Transit Score (out of 100)', 'Bike Score (out of 100)',
        ...
        def clean_price(price_str):
            ...
            return price_str  # Return the value as-is if it's already a float

        def clean_numerical(num_str):
            if isinstance(num_str, str):
                return float(num_str.replace(',', '').strip())
            return num_str  # Return the value as-is if it's already a float

        # Clean the 'Price' and 'HOA Dues' columns
        data['Price'] = data['Price'].apply(clean_price)
        data['HOA Dues'] = data['HOA Dues'].apply(clean_hoa)

        # Clean the other numerical columns
        for col in numerical_cols:
            data[col] = data[col].apply(clean_numerical)

        categorical_cols = [
            'Property Type', 'Has Pool', 'Has Patio', 'Flood Factor',
            'Fire Factor', 'Heat Factor', 'Wind Factor'
        ]
        ...
        Training_data = X_train  # Set training_data to X_train
        original_data = data  # Set original_data to data
...
def find_deals():
    global Training_data, original_data, imputer, model
...
# Now you can call this function, passing your pipeline
...
def get_feature_names(column_transformer):
    ...
    for transformer_info in column_transformer.transformers_:
        ...
        # Handle other transformers (assuming they don't change feature count)
        else:
            feature_names.extend(columns)  # or some logic to get feature names
...
def feature_importances():
    ...
    # Get the preprocessed feature names from the trained pipeline
    features = get_feature_names(clf)  # Here, you pass clf instead of pipeline
...
def summary():
    ...
    # Performance metrics
    # Assume metrics is a dictionary with keys: 'mae', 'mse', and 'r2'
    mae = metrics.get('mae', 'N/A')
    mse = metrics.get('mse', 'N/A')
    r2 = metrics.get('r2', 'N/A')
    
    return render_template('summary.html', feature_importances=feature_importances, mae=mae, mse=mse, r2=r2)
